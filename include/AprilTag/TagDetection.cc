...
namespace AprilTags {
	...
	Eigen::Matrix4d TagDetection::getRelativeTransform(double tag_size, double fx, double fy, double px, double py) const {
		...
		//rvec and tvec brings points from the model coordinate system to the camera coordinate system.
		cv::solvePnP(objPts, imgPts, cameraMatrix, distParam, rvec, tvec, 0, cv::SOLVEPNP_ITERATIVE);

		//{
		//std::string r;
		//int type = 5;// rvec.type();
		...
		T.row(3) << 0, 0, 0, 1;
		//std::cout<<"test:"<<tvec<<std::endl;
		//std::cout<<"angle:"<<atan2(wRo(1, 0), wRo(0, 0))<<std::endl;
	  //	std::cout<<"srcT:"<<T.inverse()<<std::endl;
		return T;
	}
